...
const SECTIONS: Record<string, Sym[]> = {
  ...
  Physics: [
    ...
    {
      label: "F=ma",
      latex: "VAR_1 = VAR_2 \\times VAR_3",
      vars: ["Force (F)", "Mass (m)", "Acceleration (a)"],
    },
    {
      label: "v=u+at",
      latex: "VAR_1 = VAR_2 + VAR_3 t",
      vars: ["v", "u", "a"],
    },
    {
      label: "s=ut+½at²",
      latex: "VAR_1 = VAR_2 t + \\frac{1}{2}VAR_3 t^2",
      vars: ["s", "u", "a"],
    },
  ...
  Chemistry: [
    ...
    {
      label: "pH=−log",
      latex: "\\text{pH} = -\\log[VAR_1]",
      vars: ["Ion (e.g. H⁺)"],
    },
    {
      label: "n=m/M",
      latex: "n = \\frac{VAR_1}{VAR_2}",
      vars: ["mass (m)", "Molar mass (M)"],
    },
    // ── Reaction arrows ────────────────────────────────────────────
    { label: "→", latex: "\\rightarrow" },
...
export function MathPicker({ editor }: MathPickerProps) {
  ...
  const insert = (sym: Sym) => {
    if (sym.vars && sym.vars.length > 0) {
      setPendingFormula({ sym, values: sym.vars.map(() => "") });
      return;
    }
  ...
  const submitFormula = () => {
    ...
    pendingFormula.values.forEach((val, i) => {
      latex = latex.replace(
        new RegExp(`VAR_${i + 1}`, "g"),
        val || `VAR_${i + 1}`,
      );
    });
    const wrapped = pendingFormula.sym.block
      ? `$$\n${latex}\n$$`
      : `$${latex}$`;
    editor.chain().focus().insertContent(wrapped).run();
    setPendingFormula(null);
      ...
      <button
        ...
        className={`flex h-[32px] min-w-[32px] px-1.5 items-center justify-center rounded-[6px] transition-all duration-[.4s] font-mono text-sm font-semibold ${
          open
            ...
            : "bg-white text-[#98A2B3] hover:text-[#007fff]"
        }`}
      >
        fx
      </button>
          ...
          {pendingFormula ? (
            <div className="flex flex-col gap-3 p-3 flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-[#344054]">
                  {pendingFormula.sym.label}
                </span>
                <code className="ml-auto text-[10px] text-[#98A2B3] bg-[#F9FAFB] px-1 rounded">
                  ${pendingFormula.sym.latex}$
                </code>
              </div>
                ...
                {pendingFormula.sym.vars!.map((varName, i) => (
                  <div key={i} className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const next = [...pendingFormula.values];
                      ...
                      className="w-full rounded-[6px] border border-[#D0D5DD] px-2.5 py-1.5 text-sm text-[#344054] outline-none focus:border-[#007FFF]"
                    />
                  </div>
                ))}
              </div>
              <button
                ...
                className="mt-1 w-full rounded-[6px] bg-[#007FFF] text-white text-xs font-medium py-1.5 hover:bg-[#0066CC] transition-colors"
              >
                Insert
              </button>
            </div>
            ...
            <>
              ...
              <div className="overflow-y-auto p-2 grid grid-cols-6 gap-1">
                {SECTIONS[tab].map((sym, i) => (
                  <button
                    ...
                    title={
                      sym.vars
                        ? `Fill in: ${sym.vars.join(", ")}`
                        : `$${sym.latex}$`
                    }
                    onClick={() => insert(sym)}
                    className={`h-9 flex items-center justify-center rounded-[4px] text-sm font-medium text-[#344054] hover:bg-[#DBEDFF] hover:text-[#007FFF] transition-colors overflow-hidden px-0.5 leading-none ${sym.vars ? "bg-[#F0F7FF]" : "bg-[#F9FAFB]"}`}
                  >
                    {sym.label}
                  </button>
                ))}
              ...
              <div className="border-t border-[#F0F2F5] px-3 py-1.5 text-[10px] text-[#98A2B3] shrink-0">
                Tip: type{" "}
                <code className="bg-[#F9FAFB] px-0.5 rounded">$...$</code> for
                inline math,{" "}
                <code className="bg-[#F9FAFB] px-0.5 rounded">$$...$$</code> for
                block. <span className="text-[#007FFF]">Blue</span> = fill-in
                templates.
              </div>
            </>
          )}
