    ...
    <Modal isOpen onClose={onClose} className="rounded-2xl w-full max-w-sm">
      <div className="p-6">
        ...
        <p className="text-sm text-[#667085] mb-5">
          Set a suspension end date for <span className="font-medium text-[#344054]">{name}</span>.
          They will be blocked until this date.
        </p>
...
export default function Students() {
  const {
    students, studentsTotal, studentsPage, loadingStudents,
    searchTerm, setSearchTerm,
    fetchStudents, deactivateStudent, reactivateStudent, unsuspendStudent,
  ...
  const totalPages = Math.ceil(studentsTotal / 20);
  ...
  const columns: Column<IAdminStudentListItem>[] = [
    ...
    },
    {
      ...
      header: "Questions",
      render: (s) => (
        <span className="text-sm text-[#344054]">{s.totalQuestionsSolved.toLocaleString()}</span>
      ),
    },
    {
      key: "overallAccuracy",
      ...
      <div className="bg-white rounded-2xl flex flex-col" style={{ boxShadow: CARD_SHADOW }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F2F5]">
          <div>
            ...
            <p className="text-sm text-[#667085]">{studentsTotal} total</p>
          </div>
        </div>
        ...
        <DataTable
          ...
          keyExtractor={(s) => s.id}
          emptyMessage="No students yet. They will appear here once they register."
